...
const CONFIG_DOCTOR_DEFAULT_SETTINGS: &str = concat!(
    ...
    "  agent.bypass_permission_prompts           false         default\n",
    "  agent.unattended_permission_wait_ms       300000        default\n",
    "  agent.deny_unattended_permission_prompts  false         default\n",
    ...
    "  subagents.check_interval                  32            default\n",
    ...
    "  team.first_checkpoint_seconds             3600          default\n",
    ...
    let cases = vec![
        {
            ...
            let dependencies = base_dependencies(&temporary);
            Case {
                // Every provider's catalog, each row qualified. The Phase-0
                // baseline listed only the OpenAI API snapshot under bare
                // identifiers, which named neither the provider a row belonged
                // to nor the two catalogs it never showed.
                name: "models lists every provider's bundled snapshot",
                argv: argv(&["models"]),
                dependencies,
