...
/// Admission of queued runs to slots.
///
/// The core never admits a run itself. It pauses admission on the user's
/// request and tells the scheduler when a run reached `queued`, which is the
/// only thing an approval, an answer or a retry changes for it.
