...
impl ControlPlaneStore {
    ...
    /// Every run in one state, across every repository, oldest first.
    ///
    /// The daemon serves N projects from one database, so the timer wheel and
    /// boot reconciliation sweep by state rather than by repository: a parked
    /// run is due when its provider's reset arrives, whichever project it
    /// belongs to.
