...
| `agens-server` | The machine's daemon: its single-instance guard, its runtime, the one named crossing into synchronous code, and the coordinator — the service core, the state machines, the gates, the scheduler and the timer wheel. | A project. One daemon serves many. |
...
- `agens-server` depends on `agens-core`, `agens-store`, `agens-tools`, `agens-diagnostics` and `agens-providers`. It owns the daemon: the single-instance runtime, the coordinator's state machines, its service core, its gates, the scheduler and the timer wheel. None of that belongs to a command surface, so `serve` stays a thin adapter over this crate.
  ...
  **The runtime files.** `serve.sock`, `serve.pid`, `serve.log` and `serve.lock` all derive from the data directory and never from a working directory or a project root, because the daemon is one process per machine serving N projects. The socket and the pid belong to the single-instance guard, which removes both on the way out. That is what lets `serve stop` and `serve status` find a daemon nobody told them about.

  **The pid is the readiness edge.** The lock says a daemon owns the machine's slot; the pid says it is serving, and the two are deliberately different moments. It is written once the socket is bound, the coordinator composed and the control plane open, so whoever finds a pid finds a daemon that can answer. A socket cannot carry that: it accepts `connect` from the moment it is bound, which is before the coordinator behind it exists, so a start that waited on the socket would hand back a daemon whose journal no client can read yet. The lock still answers for the interval between the two, which is what `serve status` reports as `starting`.

  **Detaching is the command surface's.** `agens serve` returns the terminal by re-executing the binary as `serve --foreground` in its own session, waiting for that pid, and only then reporting. The daemon crate has one way to run a daemon, and it blocks; which process holds the terminal is a decision about a command line, so it stays in `agens-cli`. Re-executing rather than forking is deliberate: the process that reaches that point already has threads, and a fork inherits their locks without the threads that would release them.
