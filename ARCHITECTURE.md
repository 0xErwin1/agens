...
- `agens-server` depends on `agens-core`, `agens-store` and `agens-tools`. It owns the daemon: the single-instance runtime, the coordinator's state machines, its service core, its gates, and the scheduler and timers as they land. The service core is one API with two facades: Praetor's `team_*` tools and the clients' gRPC surface differ only in transport and in the principal they arrive as, and the authorization table that decides what a principal reaches lives in the core rather than in either facade, so adding a transport cannot widen anyone's authority. None of that belongs to a command surface, so `serve` stays a thin adapter over this crate. The store dependency is the coordinator's: its state machines are the sole writer of the control-plane tables, and every transition they apply is a conditional state change plus its journal entries in one transaction. Reaching that through a port would put the transaction boundary on the wrong side of the seam, and copying the SQL into the daemon would give the control plane a second writer. The tools dependency is the gates': they re-derive the repository's topology from git, and `agens-tools` already owns the one hardened way this workspace reaches git. A second git surface inside the daemon would be a second set of answers to which configuration, environment and hooks a git invocation may execute. The gRPC facade lives in the same crate: its `.proto` is `crates/agens-server/proto/`, and its build script compiles it into `OUT_DIR` with `protox` so the codegen needs no `protoc` on the machine. Generated code is never committed, because a checked-in copy is a second definition of the wire that can disagree with the `.proto` beside it. The facade binds a unix socket and loopback and carries no authentication of its own: identity and network authorization are Custos's, and remote access is an SSH tunnel.
