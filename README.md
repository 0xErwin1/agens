...
**What the automatic grant costs.** The read-class grant is bounded by the `[permissions]` rules that name each tool, and by nothing else. `read`, `list` and `search` are matched on the path they are given. `grep` is matched on its pattern and on the path it is given, and a `deny` on either refuses the call — so `deny grep(**/.env)` keeps a subagent out of that file, while `deny read(**/.env)` alone does not, because a rule names one tool. Two shapes are not reached by a path rule at all: `glob`, which is matched on its pattern and reports file paths rather than their contents, and `bash`, which is matched on its command line. Granting `bash` grants a way around every path rule.

**Bounding a search.** A `grep` given no path — or rooted at the worktree itself — reads whatever it walks, so any `grep` deny refuses it; name a path to scope the search. A `grep` rooted at a directory is matched on that directory and not on the files under it, so a deny naming one file below it does not stop a search rooted above it.
