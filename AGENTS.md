...
- **Surfaces do not own reusable behavior.** A surface (`agens-tui`, `agens-tui-app`, CLI UX, or a future web UI) only draws, maps input to calls, and presents results. It must not own domain rules, durable state machines, or persistence (SQLite, files, or dual-write “memory then disk”). Put that in the crate that owns it (`agens-core` for contracts and pure state, `agens-store` for SQLite, providers/tools/session as today). The surface **calls** those APIs (or a trait they own); it does not reimplement them. If deleting the TUI would delete the ability to remember, stash, run a turn, or persist, the logic is in the wrong place.
